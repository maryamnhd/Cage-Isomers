...
def transformation_library(FGs):
    ...
    tFGs.rotate([ 0,0,-1], [0,0,1],center=[0,0,0])
    tFGs.rotate(45,'z')
    for deg in range(4):
        if deg!=0:
            tFGs.rotate(90,'z') 
...
cage = stk.ConstructedMolecule(
    topology_graph=stk.cage.EightPlusSixteen(building_blocks=(Zr_atom, bb1),optimizer=stk.Collapser(step_size=0.01, distance_threshold=5)),   #, distance_threshold=5
)

# Write to file
writer = stk.PdbWriter()
writer.write(molecule=cage, path='base_structure/Tet8Di16.pdb')
...
cage = read("base_structure/Tet8Di16.pdb")
...
nisomers = count_unique_isomers(transform, nLinkers, nFGs)
